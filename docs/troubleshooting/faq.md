...
### How do I update DClaw Offboard?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp offboard --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Offboard without Kubernetes?

Yes, for local development:

```bash
cd dclaw-offboard/frontend && npm run dev
cd dclaw-offboard/backend && uvicorn dclaw_offboard.main:app --reload
```
