...
# Check events
kubectl get events -n dclaw-offboard --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-offboard deployment/dclaw-offboard-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-offboard-db-credentials -n dclaw-offboard
   ```
